...
export async function addDelivery(formData: FormData) {
  ...
  if (order) {
    const newReceived = (order.received_quantity || 0) + quantity;
    const newStatus = newReceived >= order.ordered_quantity ? "completed" : "partial_delivery";
